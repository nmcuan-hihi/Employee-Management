...
import { View, Text, TouchableOpacity, StyleSheet, Dimensions, Modal, FlatList, Pressable, Alert } from 'react-native';
...
export default function MarkAttendance() {
  ...
  const [employeeCheckStates, setEmployeeCheckStates] = useState([]);
  ...
  useEffect(() => {
    ...
    const data = manageAttendance.getArrAttendanceSheetAPI();
  ...
  const fetchEmployees = async () => {
    ...
    setEmployeeCheckStates(Array(data.length).fill(false));
  };
  ...
  const selectAll = () => {
    ...
    setEmployeeCheckStates(Array(employeesData.length).fill(!selectAllChecked));
  };
  ...
  const markAttendances = () => {
    employeeCheckStates.forEach((isChecked, index) => {
      if (isChecked) {
        ...
        manageAttendance.addAttendanceSheetAPI(maNV, selectedDate, selectedTimeIn, selectedTimeOut);
        const data = manageAttendance.getArrAttendanceSheetAPI();
        setAttendanceData(data);
      }
    });

    if (employeeCheckStates.some((isChecked) => isChecked)) {
      Alert.alert('Success', 'Successfully marked attendance!');
    }
  };
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.item}>
      ...
      <Checkbox
        ...
        value={employeeCheckStates[index]}
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.dateContainer}>
          ...
          <View style={styles.timeContainer}>
            <Text>In</Text>
            <TouchableOpacity style={{ padding: 5 }} onPress={() => showTimePickerFor('In')}>
              <Text style={styles.text}>{selectedTimeIn}</Text>
            </TouchableOpacity>
            <Text>, Out</Text>
            <TouchableOpacity style={{ padding: 5 }} onPress={() => showTimePickerFor('Out')}>
        ...
        >
          <View style={styles.modalBackground}>
            <View style={styles.modalContainer}>
              ...
              <CalendarPicker width={320} onDateChange={handleDateChange} />
            </View>
          </View>
        </Modal>
