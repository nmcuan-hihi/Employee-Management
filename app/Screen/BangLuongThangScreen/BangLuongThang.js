...
export default function DanhSach() {
  ...
  const [selectedMonth, setSelectedMonth] = useState(4); // Tháng mặc định
  ...
  useEffect(() => {
    const calculateSalary = (totalHours, mucLuong) => {
      ...
      return luongThang ? luongThang : null;
    };

    //lọc nhân viên có lương hay không
    const employeesWithSalary = employees.filter((employee) => {
      const totalHours = ManageAttendance.calculateTotalWorkingHours(employee.maNV, selectedMonth, selectedYear);
      return calculateSalary(totalHours, employee.mucLuong) !== null;
    });


    const salaryList1 = employeesWithSalary.map((employee) => {
      const totalHours = ManageAttendance.calculateTotalWorkingHours(employee.maNV, selectedMonth, selectedYear);
